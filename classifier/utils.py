...
class BERTEmbExtractor(BaseEstimator, TransformerMixin):
    ...
    def bert_extract(self, X, col):
        ...
        for start in tqdm(range(0, total_samples, self.batch_size)):
            ...
            column_tokens = {
                k: torch.tensor(v).to(self.device) for k, v in tokenized.items()
            }
            ...
            df_emb = pd.DataFrame(
                emb, columns=[f"{col}_{i}" for i in range(emb.shape[1])]
            )
...
def find_best_ccp_aplpha(X_train, y_train, X_val, y_val):
    ...
    ccp_alphas_select = (
        ccp_alphas[np.linspace(0, len(ccp_alphas[:-15]), 30).astype(int)].tolist()
        + ccp_alphas[-15:].tolist()
    )
    ...
    ax.plot(
        ccp_alphas_select, test_scores, marker="o", label="test", drawstyle="steps-post"
    )
    ax.legend()
    plt.show()
    ...
    print(
        f"best accuracy val: train {train_scores[best_hyper]} ,{test_scores[best_hyper]}"
    )
...
def metrics(pipe, X_val, y_val, LE):
    ...
    ConfusionMatrixDisplay.from_estimator(
        ...
        X_val,
        y_val,
        display_labels=LE.classes_[display_labels],
