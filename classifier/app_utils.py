...
def request_inn(company_inn):
    all_data_about_company = requests.get(
        f"https://api.ofdata.ru/v2/company?key={ofdata_token}&inn={company_inn}"
    )
...
def extract_add_data(all_data_about_company, additional_data):
    ...
    additional_data["Уставный капитал, тип"] = all_data_about_company["data"][
        "УстКап"
    ].get("Тип")

    additional_data["Уставный капитал, сумма"] = all_data_about_company["data"][
        "УстКап"
    ].get("Сумма")

    additional_data["Вид экономической деятельности, ОКВЭД"] = all_data_about_company[
        "data"
    ]["ОКВЭД"].get("Наим")

    if len(all_data_about_company["data"]["ОКВЭДДоп"]) >= 1:
        additional_data["Доп вид экономической деятельности_1"] = (
            all_data_about_company["data"]["ОКВЭДДоп"][0].get("Наим")
        )

    if len(all_data_about_company["data"]["ОКВЭДДоп"]) >= 2:
        additional_data["Доп вид экономической деятельности_2"] = (
            all_data_about_company["data"]["ОКВЭДДоп"][1].get("Наим")
        )

    if len(all_data_about_company["data"]["ОКВЭДДоп"]) >= 3:
        additional_data["Доп вид экономической деятельности_3"] = (
            all_data_about_company["data"]["ОКВЭДДоп"][2].get("Наим")
        )

    additional_data["Среднесписочная численность сотрудников"] = all_data_about_company[
        "data"
    ].get("СЧР")

    additional_data["Категория из реестра СМП"] = all_data_about_company["data"][
        "РМСП"
    ].get("Кат")

    additional_data["Сумма уплаченных налогов за 2020"] = all_data_about_company[
        "data"
    ]["Налоги"].get("СумУпл")

    additional_data["Тип по ОКОГУ"] = all_data_about_company["data"]["ОКОГУ"].get(
        "Наим"
    )
    return additional_data
...
def select_workplaces(workplace, additional_data):
    ...
    company_names = [
        fns_data[i]["value"] for i in range(number_of_companies)
    ]  # [fns_data[i]['data']['name']['full_with_opf'] for i in range(number_of_companies)]
    company_adresses = [
        fns_data[i]["data"]["address"]["unrestricted_value"]
        for i in range(number_of_companies)
    ]

    company_names_and_adresses = []
    for i in range(len(company_names)):
        company_names_and_adresses.append(
            company_names[i] + " (" + company_adresses[i] + ")"
        )

    company_name = st.multiselect(
        "Выберете компанию, подходящую под ваше название", company_names_and_adresses
    )
...
def visualize(df):
    logins_to_show_career_track = st.multiselect(
        "Выберете логин, чью карьерную траекторию отображать:",
    ...
    df["Карьерная ступень по Классификатору ФОИР"] = df[
        "Карьерная ступень по Классификатору ФОИР"
    ].astype("int")
    df["Сфера деятельности по Классификатору ФОИР"] = (
        df["Сфера деятельности по Классификатору ФОИР"]
        .apply(lambda x: x[0])
        .astype("str")
    )
    ...
    for login in logins_to_show_career_track:
        fig.add_trace(
            go.Scatter(
                x=df[df["Номер"] == login]["Начало периода должности"],
        ...
        colors_d = df[df["Номер"] == login][
            "Сфера деятельности по Классификатору ФОИР"
        ].map(color_map)  # создаём цветовую схему, соответствующую "Сфере деятельности"
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    color=colors_d,  # используем цветовую схему, соответствующую "Сфере деятельности"
                    size=9,
    ...
    st.markdown(
        f"""
                {img_to_html(f'{color_path}красный.png')} - Федеральные государственные органы, в т.ч. федеральная государственная служба; 
                \n{img_to_html(f'{color_path}оранжевый.png')} - Региональные органы государственной власти, в т.ч. гражданская служба субъектов РФ; 
                \n{img_to_html(f'{color_path}жёлтый.png')} - Органы местного самоуправления (ОМСУ); 
                \n{img_to_html(f'{color_path}серый.png')} - Подведомственные органам власти (государственные/муниципальные) организации;
                \n{img_to_html(f'{color_path}зелёный.png')} - Общественные/общественно-политические организации и НКО;
                \n{img_to_html(f'{color_path}коричневый.png')} - Государственные корпорации и Институты развития;
                \n{img_to_html(f'{color_path}синий.png')} - Коммерческие компании, государственные/муниципальные предприятия;
                """,
...
def search_regexp(dataframe, regex_for_three_gov_fields, matching_three_gov_fields):
    ...
    for i, _ in dataframe.iterrows():
        ...
        for field, fields_list_of_regex in regex_for_three_gov_fields.items():
            for regex_index in range(len(list(fields_list_of_regex))):
                finded = re.search(
                    list(fields_list_of_regex)[regex_index],
                    "".join(dataframe.loc[i, "Место работы"].lower().split()),
                )
    ...
    dataframe["Сфера деятельности по regex"] = dataframe[
        "Сфера деятельности по regex"
    ].map(matching_three_gov_fields)
